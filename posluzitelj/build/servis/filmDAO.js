...
export class FilmDAO {
    ...
    async dohvatiFilm(filmId) {
        const sql = 'SELECT * FROM film WHERE id = ?';
        const film = await this.baza.izvrsiUpitPromiseAll(sql, [filmId]);
        return film;
    }
    async dohvatiFilmoveStranica(stranica) {
        const offset = (stranica - 1) * 20;
        const sql = `
            SELECT * FROM film 
            LIMIT 20 OFFSET ?
        `;
        const filmovi = await this.baza.izvrsiUpitPromiseAll(sql, [offset]);
        //console.log(filmovi);
        return filmovi;
    }
    async dodajFilmoveOsobi(id_osoba, filmovi) {
    ...
    async dohvatiFilmoveOsobe(id_osoba, stranica) {
        let offset;
        stranica == 0 ? offset = 0 : offset = (stranica - 1) * 20;
        if (offset == 0) {
            var sql = "SELECT f.* FROM film f JOIN uloga u ON f.id = u.id_film WHERE u.id_osoba = ?";
            var filmovi = await this.baza.izvrsiUpitPromiseAll(sql, [id_osoba]);
        }
        else {
            var sql = "SELECT f.* FROM film f JOIN uloga u ON f.id = u.id_film WHERE u.id_osoba = ? LIMIT 20 OFFSET ?";
            var filmovi = await this.baza.izvrsiUpitPromiseAll(sql, [id_osoba, offset]);
        }
        return filmovi;
    }
    async dohvatiFilmoveStranicaDatumi(stranica, datumOd, datumDo) {
        const offset = (stranica - 1) * 20;
        ...
        const filmovi = await this.baza.izvrsiUpitPromiseAll(sql, params);
        return filmovi;
    }
    ...
    async obrisiFilmoveOsobe(id_osoba) {
