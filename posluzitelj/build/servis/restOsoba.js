...
export class RestOsoba {
    ...
    deleteOsobuBezId(zahtjev, odgovor) {
        odgovor.type("application/json");
        odgovor.status(405).send(JSON.stringify({ poruka: "zrbranjena metoda" }));
    }
    ...
    async getOsobeStranica(zahtjev, odgovor) {
    ...
    async getFilmoveOsobe(zahtjev, odgovor) {
        odgovor.type("application/json");
        let id = parseInt(zahtjev.params["id"] || "0");
        let osoba = await this.osobaDAO.dohvatiOsobu(id);
        if (id == 0)
            odgovor.status(400).send({ greska: "Nisu poslani parametar id" });
        else {
            let stranica = parseInt(zahtjev.query["stranica"]);
    ...
    async putFilmoveOsobi(zahtjev, odgovor) {
        odgovor.type("application/json");
        let id = parseInt(zahtjev.params["id"] || "0");
        if (id == 0) {
    ...
    async deleteFilmoveOsobe(zahtjev, odgovor) {
        odgovor.type("application/json");
        let id = parseInt(zahtjev.params["id"] || "0");
        if (id == 0) {
            odgovor.status(400).json({ greska: "Nisu poslani parametar id" });
    ...
    async postOsobaStranica(zahtjev, odgovor) {
        ...
        if (dodanaOsoba)
            odgovor.status(201).json(dodanaOsoba);
        else
            odgovor.status(400).json({ greska: "Nije moguće dodati osobu" });
    }
    async deleteOsobu(zahtjev, odgovor) {
        let id = parseInt(zahtjev.params["id"] || "0");
        if (id == 0) {
            odgovor.status(400).json({ greska: "Nisu poslani parametar id" });
        }
