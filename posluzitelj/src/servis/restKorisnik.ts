import { Ikorisnik, Zahtjev } from "../Inteface.js";
...
export class RestKorisnik {
	...
	getKorisnici(zahtjev: Request, odgovor: Response) {
		...
		this.kdao.dohvatiSveKorisnike().then((korisnici: Array<Ikorisnik>) => {
	...
	async postKorisnici(zahtjev: Request, odgovor: Response) {
		...
		if(poruka) odgovor.status(200);
		else odgovor.status(400);
	...
	putKorisnik = async (zahtjev: Request, odgovor: Response) => {
		odgovor.type("application/json");
		odgovor.status(405);
		let poruka = { greska: "zabranjena metoda" };
		odgovor.send(JSON.stringify(poruka));
	...
	deleteKorisnik(zahtjev: Request, odgovor: Response) {
        odgovor.type("application/json");
        var korime = zahtjev.params["korime"] as string;
	...
	provjeriPristup(zahtjev: Request, odgovor: Response) {
        odgovor.type("application/json");
        var korime = zahtjev.params["korime"] as string;
	...
	async dohvatiZahtjev(zahtjev: Request, odgovor: Response) {
		odgovor.type("application/json");
		const korime = zahtjev.params["korime"] as string;
		//console.log("korime iz dohvatiZahtjev rest:", korime);

		try {
			const zahtjev = await this.kdao_servis.dajZahtjev(korime) as Zahtjev;
	...
	async dodajZahtjev(zahtjev: Request, odgovor: Response) {
        odgovor.type("application/json");
        const korime = zahtjev.params["korime"] as string;
	...
	async obrisiZahtjev(zahtjev: Request, odgovor: Response) {
        odgovor.type("application/json");
        const korime = zahtjev.params["korime"] as string;
	...
	async izbrisiPristup(zahtjev: Request, odgovor: Response) {
		odgovor.type("application/json");
		const korime = zahtjev.params["korime"] as string;
