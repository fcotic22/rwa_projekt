import { Ikorisnik, Zahtjev } from "src/Inteface.js";
...
export class KorisnikDAO{
    ...
    async dohvatiSveKorisnike() : Promise<Array<Ikorisnik>>{
       
        const sql = 'SELECT * FROM korisnik';
        var korisnici = await this.baza.izvrsiUpitPromiseAll(sql, []) as Array<Ikorisnik>;
    
        var rezultat = new Array<Ikorisnik>();
        for(var kor of korisnici){
            let k : Ikorisnik = {ime: kor["ime"], prezime: kor["prezime"], adresa: kor["adresa"], datum_rodjenja: kor["datum_rodjenja"], spol: kor["spol"] , korime: kor["korime"], lozinka: kor["lozinka"], email: kor["email"], tip_korisnika_id: kor["tip_korisnika_id"] , tajniTOTP: kor["tajniTOTP"], aktivacijski_kod: kor["aktivacijski_kod"], aktivan: kor["aktivan"]};
            rezultat.push(k);
        }
        return rezultat;
    }
    ...
    async dohvatiKorisnika(korime : string) : Promise<Ikorisnik|null>{
        ...
        var korisnik = (await this.baza.izvrsiUpitPromiseAll(sql, [korime]) as Array<Ikorisnik>)[0];
    ...
    async azurirajKorisnika(korime: string, korisnik: Ikorisnik): Promise<boolean> {
    ...
    async ukljuciAuth(korime: string, tajniTOTP: string): Promise<boolean> {
    ...
    async iskljuciAuth(korime: string): Promise<boolean> {
    ...
    async oznaciTOTPVerifikaciju(korime: string): Promise<boolean> {
...
export class KorisnikDAOservis{
	...
	async dohvatiKorisnikaServis(korime: string): Promise<Ikorisnik | null> {
		const sql = 'SELECT * FROM korisnik WHERE korime = ?';
		var korisnik = (await this.baza2.izvrsiUpitPromiseAll(sql, [korime]) as Array<Ikorisnik>)[0];
	...
	async dodajZahtjevServis(korime: string): Promise<boolean> {
    ...
    async dajZahtjev(korime: string): Promise<any> {
        const sql = 'SELECT * FROM zahtjev WHERE korime = ?';
        try {
            const zahtjev = await this.baza2.izvrsiUpitPromiseAll(sql, [korime]) as Array<Zahtjev>;
    ...
    async obrisiZahtjev(korime: string): Promise<boolean> {
